...
<body>
	 ...
	 <table class="table table-hover">
		...
		<?php
			...
			$sql = "select `Name`,`Month`,`Year`,`Paper_Path` from `student_resource_past_paper`  group by Year";
			$result = $connection->query($sql);
				if($result->num_rows>0){
					while($row = $result->fetch_assoc()){
						echo '<tr><td>'. $row["Name"].'</td><td>'. $row["Month"].'</td><td>'. $row["Year"].'</td><td><a class="btn btn-default" href="'.$row["Paper_Path"].'" download>Download</a></td></tr>';
